...
export default function SupplierDetailPage() {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      setIsLoading(true)
      try {
        const response = await fetch(`http://localhost:5005/api/v1/suppliers/${supplierId}`, { credentials: "include"})
        if (!response.ok) {
  ...
  const onSubmit = async (data: SupplierFormValues) => {
    ...
    try {
      const response = await fetch(`http://localhost:5005/api/v1/suppliers/${supplierId}`, {
        method: "PATCH",
         credentials: "include",
        headers: {
