...
export default function EditTailoringProject() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/tailoring/${id}`, { credentials: "include"})
  ...
  const { data: workflowConfigs = [] } = useQuery<WorkflowConfig[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/tailoring/workflow/templates`, {  credentials: "include" })
      const json = await response.json()
      if (!response.ok) {
  ...
  const onSubmit = async (data: UpdateTailoringDto) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/tailoring/${id}`, {
        method: "PATCH",
         credentials: "include",
        headers: {
