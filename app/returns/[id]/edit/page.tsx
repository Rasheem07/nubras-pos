...
export default function EditReturnPage() {
  ...
  useEffect(() => {
    const loadReturnData = async () => {
      try {
        const response = await fetch(`http://3.29.240.212/api/v1/returns/${returnId}`)
        if (!response.ok) {
  ...
  const loadOrderItems = async () => {
    ...
    try {
      const response = await fetch(`http://3.29.240.212/api/v1/sales/${returnData.orderId}/items`)
      if (!response.ok) {
