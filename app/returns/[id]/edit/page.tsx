...
export default function EditReturnPage() {
  ...
  useEffect(() => {
    const loadReturnData = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/returns/${returnId}`, { credentials: "include"})
        if (!response.ok) {
  ...
  const loadOrderItems = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/sales/${returnData.orderId}/items`, { credentials: "include",})
      if (!response.ok) {
  ...
  const onSubmit = async (data: ReturnFormData) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/returns/${returnId}`, {
         credentials: "include",
        method: "PATCH",
        headers: {
